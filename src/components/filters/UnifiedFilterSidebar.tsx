 ...
 *
 * Advanced filter sidebar with search, pending state, localStorage persistence,
 * and expand/collapse all functionality. Supports both desktop and mobile modes.
 * Framework-agnostic with injectable icons.
 *
 * Features:
 * - Filter search with highlighting
 * - Pending state (apply changes on button click)
 * - localStorage persistence for collapsed sections
 * - Expand/Collapse all buttons
 * - Mobile/desktop responsive
 * - Inline search capability
 * - PriceRangeFilter integration
 *
 * @example
 * ```tsx
...
export interface UnifiedFilterSidebarProps {
  ...
  /** Callback when a filter value changes (immediate mode) */
  ...
  searchable?: boolean;
  /** Mobile mode (slide-in, different layout) */
  mobile?: boolean;
  /** Show inline search input (separate from filter search) */
  showInlineSearch?: boolean;
  /** Inline search value */
  inlineSearchValue?: string;
  /** Callback when inline search changes */
  onInlineSearchChange?: (value: string) => void;
  /** Placeholder for inline search */
  inlineSearchPlaceholder?: string;
  /** Custom search icon (injectable) */
  SearchIcon?: React.ComponentType<{ className?: string }>;
  /** Custom X/close icon (injectable) */
...
export const UnifiedFilterSidebar: React.FC<UnifiedFilterSidebarProps> = ({
  ...
  const filteredSections = useMemo(() => {
    ...
    return sections
      ...
      .filter(Boolean) as FilterSidebarSection[];
