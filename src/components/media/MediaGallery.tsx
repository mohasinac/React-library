/**
 * MediaGallery - Generic Media Gallery Component
 *
 * A reusable media gallery component for any resource type (products, auctions, etc.)
 * with full lightbox support including zoom, rotate, and download features.
 *
 * Features:
 * - Image/video carousel with auto-play
 * - Lightbox with zoom (50%-300%), rotate (90°), and download
...
export interface MediaGalleryProps {
  ...
  media: GalleryMedia[];
  /** Resource name for alt text and downloads */
  resourceName: string;
  /** Image component (Next.js Image, img, etc.) */
  ...
  /** Auto-play slideshow interval in milliseconds */
...
export function MediaGallery({
  ...
  const handleDownload = async () => {
    if (activeMedia.type === "video") return; // Skip videos
    ...
    try {
      ...
      link.href = url;
      link.download = `${resourceName}-${activeIndex + 1}.jpg`;
  ...
  return (
    <>
      <div className={`space-y-4 ${className}`}>
        ...
        <div className="relative aspect-square bg-gray-50 dark:bg-gray-800 rounded-lg overflow-hidden group">
          {activeMedia.type === "image" ? (
            <ImageComponent
              src={activeMedia.url}
              alt={activeMedia.alt || resourceName}
              fill
            ...
            {media.map((item, index) => (
              ...
              >
                {item.type === "image" ? (
                  <ImageComponent
                    src={item.url}
                    alt={`${resourceName} thumbnail ${index + 1}`}
                    fill
      ...
      {/* Lightbox - Lightroom style with zoom, rotate, download */}
      {isLightboxOpen && (
        <div
        ...
        >
          ...
          <button
            onClick={() => setIsLightboxOpen(false)}
            className="absolute top-4 right-4 z-[10001] bg-black/60 hover:bg-black/80 backdrop-blur-sm p-3 rounded-full text-white shadow-2xl focus:outline-none focus:ring-2 focus:ring-white/50 transition-all"
          ...
          <div className="absolute top-4 left-1/2 -translate-x-1/2 z-[10001] flex items-center gap-2 bg-black/60 backdrop-blur-sm rounded-full px-3 py-2 shadow-2xl">
            {activeMedia.type === "image" && (
              <>
                <button
                  onClick={handleZoomOut}
                  disabled={zoomLevel <= 0.5}
                  className="p-2 text-white hover:bg-white/10 rounded-full transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                  aria-label="Zoom out"
                  title="Zoom out"
                >
                  <svg
          ...
          {/* Content container */}
          <div className="relative z-[10000] w-full h-full max-w-7xl max-h-screen p-4 flex items-center justify-center overflow-hidden">
            <div
            ...
            {media.length > 1 && (
              <>
                <button
                  onClick={handlePrevious}
                  className="absolute left-4 top-1/2 -translate-y-1/2 z-[10001] bg-black/60 hover:bg-black/80 backdrop-blur-sm p-3 rounded-full text-white shadow-2xl transition-all"
                  aria-label="Previous image"
                >
                  {typeof ChevronLeftIcon === "function" ? (
                    <ChevronLeftIcon />
                ...
                <button
                  onClick={handleNext}
                  className="absolute right-4 top-1/2 -translate-y-1/2 z-[10001] bg-black/60 hover:bg-black/80 backdrop-blur-sm p-3 rounded-full text-white shadow-2xl transition-all"
                  aria-label="Next image"
                >
                  {typeof ChevronRightIcon === "function" ? (
                    <ChevronRightIcon />
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-[10001] bg-black/60 backdrop-blur-sm text-white px-4 py-2 rounded-full shadow-2xl">
                  {activeIndex + 1} / {media.length}
                </div>
              </>
            )}
